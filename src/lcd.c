...
// ST756R LCD controller definitions
#define CMD_COL_UPPER			0x10	// RAM column address upper 4 bits
#define CMD_COL_LOWER			0x00	// RAM column address lower 4 bits
#define CMD_RES_RATIO			0x20	// internal resistor ratio mode
#define CMD_PWR_CTRL			0x28	// power operating mode
#define CMD_ELEC_VOL_REG		0x81	// set electronic volume register
#define CMD_SEG_NORMAL			0xA0	// RAM address SEG output
#define CMD_BIAS_1_9			0xA2	// drive voltage bias ratio: 1/9
#define CMD_ALL_PTS_OFF			0xA4	// display all points: off
#define CMD_DISP_NORMAL			0xA6	// display: normal
#define CMD_DISP_REVERSE		0xA7	// display: reverse
#define CMD_DISP_OFF			0xAE	// display: off
#define CMD_DISP_ON				0xAF	// display: on
#define CMD_PAGE				0xB0	// RAM page address
#define CMD_COM_REVERSE			0xC8	// COM output scan direction: reverse

#define RST_DLY_COUNT			100		// reset hold and completion delay count

// LcdPrintf() definitions
...
static UInt8 ShadowDisp[LCD_PAGE_HEIGHT][LCD_PIXEL_WIDTH];

static const UInt8	ByteTopMask[] = {0x00, 0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3f, 0x7f};


static void LeftFill(Bool ReverseFlag);
static void RightFill(Bool ReverseFlag);
static void CharWrite(Int CharVal, Bool ReverseFlag);
...
void LcdInit(void)
{
	...
	// Delay to hold the LCD reset asserted
	for (ResetDly = 0; ResetDly < RST_DLY_COUNT; ResetDly++)
		;

	// Deassert the LCD reset, delay to allow the LCD to come out of reset
	SpiResetDeassert();
	for (ResetDly = 0; ResetDly < RST_DLY_COUNT; ResetDly++)
		;

	// Initialize the LCD
	SpiCmdWrite(CMD_SEG_NORMAL);		// RAM address SEG output: normal
	SpiCmdWrite(CMD_DISP_OFF);			// display: off
	SpiCmdWrite(CMD_COM_REVERSE);		// COM output scan direction: reverse
	SpiCmdWrite(CMD_BIAS_1_9);			// drive voltage bias ratio: 1/9
	SpiCmdWrite(CMD_ALL_PTS_OFF);		// display all points: off
	SpiCmdWrite(CMD_PWR_CTRL + 7);		// power operating mode: 7
	SpiCmdWrite(CMD_RES_RATIO + 1);		// internal resistor ratio mode: 1:1
	SpiCmdWrite(CMD_ELEC_VOL_REG);		// set electronic volume register
	SpiCmdWrite(0x1F);					// electronic volume register value
	SpiCmdWrite(CMD_DISP_NORMAL);		// display: normal
	SpiCmdWrite(CMD_DISP_ON);			// display: on

	// Clear the display and the shadow display
	LcdClear();
}
...
void LcdClear(void)
{
	...
	// Clear the display and the shadow display
	for (DispPage = 0; DispPage < LCD_PAGE_HEIGHT; DispPage++)
	{
		// Set the column and page address to the start of the character row
		SpiCmdWrite(CMD_COL_UPPER + 0);	// RAM column address upper 4 bits: 0
		SpiCmdWrite(CMD_COL_LOWER + 0);	// RAM column address lower 4 bits: 0
		SpiCmdWrite(CMD_PAGE + DispPage);	// RAM page address: DispPage

		for (DispCol = 0; DispCol < LCD_PIXEL_WIDTH; DispCol++)
		{
			SpiDataWrite(0x00);
		}
	}
...
{
	SpiCmdWrite(ReverseDisplay ? CMD_DISP_REVERSE : CMD_DISP_NORMAL);
}


void LcdRenderObject(Int Object, Int PosX, Int PosY, Bool RenderFlag)
{
	static UInt8 ObjectImages[2][8] =
	{
		{0xff, 0xff, 0xc3, 0xc3, 0xc3, 0xc3, 0xff, 0xff},	// Gobbler
		{0x0f, 0x0f, 0x0f, 0x0f, 0x00, 0x00, 0x00, 0x00}	// Goblet
	};
	...
	UInt8	DispVal;
	Int		ImageCol;
	UInt8 *	ImageBufPtr = &(ObjectImages[Object][0]);
	Int		ObjectPixelWidth;
	Int		ObjectPixelHeight;
	Int		RenderCol;
	UInt8	RenderVal;
	Int		VertOffset;
	...
	switch (Object)
	{
	case LCD_OBJ_GOBBLER:
		ObjectPixelWidth = LCD_GOBBLER_PIXEL_WIDTH;
		ObjectPixelHeight = LCD_GOBBLER_PIXEL_HEIGHT;
		break;

	case LCD_OBJ_GOBLET:
		ObjectPixelWidth = LCD_GOBLET_PIXEL_WIDTH;
		ObjectPixelHeight = LCD_GOBLET_PIXEL_HEIGHT;
		break;
	}

	// Limit the object position to be within the display
	PosX = PosX < 0 ? 0 : PosX;
	PosX = PosX > (LCD_PIXEL_WIDTH - ObjectPixelWidth) ? LCD_PIXEL_WIDTH - ObjectPixelWidth : PosX;
	PosY = PosY < 0 ? 0 : PosY;
	PosY = PosY > (LCD_PIXEL_HEIGHT - ObjectPixelHeight) ? LCD_PIXEL_HEIGHT - ObjectPixelHeight : PosY;

	// Determine the starting column, page, and offset within the page
	DispCol = PosX;
	DispPage = PosY / LCD_PAGE_PIXEL_HEIGHT;
	VertOffset = PosY - (DispPage *  LCD_PAGE_PIXEL_HEIGHT);

	// Set up the page and column addresses for the display
	SpiCmdWrite(0x10 + (DispCol >> 4));
	SpiCmdWrite(0x00 + (DispCol & 0x0f));
	SpiCmdWrite(0xB0 + DispPage);

	// Render the object image into the top page, retaining other objects within the page
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < ObjectPixelWidth; RenderCol++, ImageCol++)
	{
		DispVal = ShadowDisp[DispPage][RenderCol];
		RenderVal = (DispVal & ByteTopMask[VertOffset]) | (ImageBufPtr[ImageCol] << VertOffset);
		if (RenderFlag == LCD_OBJ_ERASE)
		{
			RenderVal = DispVal;
		}
		SpiDataWrite(RenderVal);
	}

	// Check for being on a page boundary
	if (VertOffset == 0)
	{
		return;
	}
	...
	// Set up the page and column addresses for the display
	// A vertical offset can only occur if there is a page below the top page
	DispPage++;
	SpiCmdWrite(0x10 + (DispCol >> 4));
	SpiCmdWrite(0x00 + (DispCol & 0x0f));
	SpiCmdWrite(0xB0 + DispPage);

	// Render the object image into the bottom page, retaining other objects within the page
	for (RenderCol = DispCol, ImageCol = 0; ImageCol < ObjectPixelWidth; RenderCol++, ImageCol++)
	{
		DispVal = ShadowDisp[DispPage][RenderCol];
		RenderVal = (DispVal & ~ByteTopMask[VertOffset]) | (ImageBufPtr[ImageCol] >> (LCD_PAGE_PIXEL_HEIGHT - VertOffset));
		if (RenderFlag == LCD_OBJ_ERASE)
		{
			RenderVal = DispVal;
		}
...
int LcdPrintf(Int CharRow, Int CharCol, Bool ReverseFlag, char * FmtStrPtr, ...)
{
	...
	static const char HexDigitArray[32] =	// hex digit character lookup table
	{
	...
	CharRow = (CharRow < LCD_CHAR_HEIGHT) ? CharRow : LCD_CHAR_HEIGHT - 1;
	CharCol = (CharCol < 0) ? 0 : CharCol;
	CharCol = (CharCol < LCD_CHAR_WIDTH) ? CharCol : LCD_CHAR_WIDTH - 1;

	// Set up the row and column addresses in the LCD
	SpiCmdWrite(CMD_COL_UPPER + ((CharCol * LCD_CHAR_PIXEL_WIDTH) >> 4));	// RAM column address upper 4 bits
	SpiCmdWrite(CMD_COL_LOWER + ((CharCol * LCD_CHAR_PIXEL_WIDTH) & 0x0F));	// RAM column address lower 4 bits
	SpiCmdWrite(CMD_PAGE + CharRow);									// RAM page address
	...
	while(*FmtStrPtr)
	{
		...
		if(*FmtStrPtr != '%')
		{
			CharWrite(*FmtStrPtr++, ReverseFlag);
			OutputCharCount++;
			continue;
		}
		...
		case 'b':
			...
			LeftFill(ReverseFlag);
			while (NumCharArray[NumCharArrayIdx])
			{
				CharWrite(NumCharArray[NumCharArrayIdx++], ReverseFlag);
				OutputCharCount++;
			}
			RightFill(ReverseFlag);
			break;

		case 'c':
			FieldWidth = 1;
			LeftFill(ReverseFlag);
			CharWrite(va_arg(ArgPtr, int), ReverseFlag);
			OutputCharCount++;
			RightFill(ReverseFlag);
			break;
...
static void LeftFill(Bool ReverseFlag)
{
	...
	{
		CharWrite(IsZeroFilled ? '0' : ' ', ReverseFlag);
		OutputCharCount++;
		FillCharCount--;
	}
...
static void CharWrite(Int CharVal, Bool ReverseFlag)
{
	...
	// Chars 0x80-0xff have a space substituted for them
	CharVal = CharVal > 0x7f ? ' ' : CharVal;

	for (CharCol = 0; CharCol < LCD_CHAR_PIXEL_WIDTH; CharCol++)
	{
		ColVal = FontArray[(CharVal * LCD_CHAR_PIXEL_WIDTH) + CharCol];
		ColVal = ReverseFlag ? ~ColVal : ColVal;
		SpiDataWrite(ColVal);
	}
}
